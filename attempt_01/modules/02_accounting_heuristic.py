...
family_choices = pickle.load(open("attempt_01/artifacts/family_choices.pkl", "rb"))
preference_cost = pickle.load(open("attempt_01/artifacts/preference_cost.pkl", "rb"))
family_members = pickle.load(open("attempt_01/artifacts/family_members.pkl", "rb"))
...
output_string = '{:<13{}}{:<11{}}{:<12{}}{:<11{}}{:<17{}}{:<17{}}{:<{}}'
                    ...
                    if tour_model.status == grb.GRB.OPTIMAL:
                        ...
                        if tmp_cost < current_cost:
                            ...
                            if feasible_counter % 10 == 1:
                                print('')
                                print(output_string.format('', 's', 'CHANGES', 's', 'SCHEDULE', 's',
                                                           'COST', 's', '', 's',
                                                           '', 's',
                                                           '', '11s'))
                                print(output_string.format('ITERATION', 's', 'TESTED', 's', 'FEASIBLE', 's',
                                                           'REDUCED', 's', 'CURRENT COST', 's',
                                                           'TOTAL SAVED', 's',
                                                           'RUN TIME', '11s'))
                                print('-' * 90)

                            print("\033[95m{}\033[00m".format(
                                output_string.format(i, 'd', change_counter, 'd', feasible_counter, 'd',
                                                     cost_counter, 'd', current_cost, ',d',
                                                     cost_saved, ',d', cumulative_time, '11.2f')))

                            tour_model.write('attempt_01/outputs/tour_solution.sol')
                            ...
                            print(output_string.format(i, 'd', change_counter, 'd', feasible_counter, 'd',
                                                       cost_counter, 'd', current_cost, ',d',
                                                       cost_saved, ',d', cumulative_time, '11.2f'))
                    else:
                        visit[a, b_prev].lb = 1
...
tour_model.read('attempt_01/outputs/tour_solution.sol')
all_vars = tour_model.getVars()
for v in all_vars:
    if v.start > 0.5:
        v.lb = 1
