...
# Read CSV --------------------------------------------------------
family_data = pd.read_csv('attempt_04/inputs/family_data.csv')
...
f = open("attempt_04/artifacts/family_choices.pkl", "wb")
pickle.dump(family_choices_dict, f)
f.close()

f = open("attempt_04/artifacts/preference_cost.pkl", "wb")
pickle.dump(preference_cost, f)
f.close()

f = open("attempt_04/artifacts/accounting_cost.pkl", "wb")
pickle.dump(accounting_cost, f)
f.close()

f = open("attempt_04/artifacts/family_members.pkl", "wb")
...
# Write model to file ---------------------------------------------
tour_model.write('attempt_04/artifacts/tour_model.lp')

# Set parameters --------------------------------------------------
tour_model.setParam('MIPFocus', 1)
tour_model.setParam('Cuts', 0)
tour_model.setParam('MIPGap', 0.001)

# Solve model -----------------------------------------------------
tour_model.optimize()

# Write solution to file ------------------------------------------
tour_model.write('attempt_04/outputs/tour_solution.sol')
